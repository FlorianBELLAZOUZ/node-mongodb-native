...
var connect = function(self, url, options, callback) {
  ...
  if(_finalOptions.autoReconnectFirst == null) _finalOptions.autoReconnectFirst = true;
  ...
  function connectCallback(err, db) {
    var tryReconnect = _finalOptions.autoReconnectFirst===true && err && /failed to connect to server.*on first connect/.test(err.message)

    if(tryReconnect) {
      if(logger.isWarn()) {
        logger.warn(f('try to reconnect server:port'));
      }
      setTimeout(()=>connect(self,url,options,callback),options.reconnectInterval)
    }
